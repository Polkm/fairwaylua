...
function PANEL:SetItem(tblItemTable, intAmount, strUseCommand, intCost)
	...
	if strUseCommand == "diposite" then
		self.DoUseItem = function(intAmountToDipostite)
			self:RunPromtAmount(tblItemTable, intAmount, "How many to diposite", "UD_DipostiteItem", intAmountToDipostite)
		end
	end
	...
	if tblItemTable.ClipSize then strTooltip = Format("%s\nClipsize: %s", strTooltip, tblItemTable.ClipSize) end
	...
	local menuFunc = function()
		...
		if strUseCommand == "diposite" && self.DoUseItem then GAMEMODE.ActiveMenu:AddOption("Diposite", function() self.DoUseItem() end) end
...
function PANEL:RunPromtAmount(tblItemTable, intAmount, strQuestion, strCommand, intCallAmount)
	if (tblItemTable.Stackable or intAmount >= 5) && !intCallAmount then 
