...
function SWEP:SecondaryAttack()
	...
	if self.Primary.Automatic != true then
		...
		if (SERVER) then
			local speed = self:GetOwner().Upgrades[self.Item].slvl
			local acc = self:GetOwner().Upgrades[self.Item].alvl	
			self.Primary.Delay = UpgradeLevels[self.Item].FiringSpeed[speed] - 0.05
			self.Primary.Cone = UpgradeLevels[self.Item].Accuracy[acc] + 0.03
		end
