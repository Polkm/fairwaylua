...
function CC_GMOD_Tool(player, command, arguments)
	--If there is no tool, stop the function
	if arguments[1] == nil then return end
	--Make sure the server owner is cool with it, By default its ok
	if GetConVarNumber("toolmode_allow_" .. arguments[1]) != 1 then return end
	
	--Changes the concommand so that the client knows whats up
