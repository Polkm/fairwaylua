...
function toExp(intLevel)
	local intLevel = tonumber(intLevel)
...
function GetPropClass(strModel)
	...
	if strModel && !util.IsValidProp(strModel) then strEntType = "prop_dynamic" end
	return strEntType
end
...
if CLIENT then
	...
	function CreateGenericSlider(pnlParent, strText, intMin, intMax, intDecimals, strConVar)
		...
		nmsNewNumSlider.UpdateSlider = function(intNewValue)
			nmsNewNumSlider:SetValue(intNewValue)
			nmsNewNumSlider.Slider:SetSlideX(nmsNewNumSlider.Wang:GetFraction())
		end
